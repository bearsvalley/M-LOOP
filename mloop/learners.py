...
class GaussianProcessLearner(Learner, mp.Process):
    ...
    def __init__(self, 
            ...
            #     parameter_searches = 0,
                 ...
                 bias_func_cycle = 0,
                 bias_func_cost_factor=[],
                 bias_func_uncer_factor=[],
                 search_precision = 0,
                 bad_uncer_frac = 0, 
                 **kwargs):
        ...
        if gp_training_filename is not None:
            ...
            num_params_init = kwargs.pop('num_params',None)  #Unused Variable
            min_boundary_init = kwargs.pop('min_boundary',None) # Unused Variable
            max_boundary_init = kwargs.pop('max_boundary',None) # Unused Variable
            ...
            self.all_status = np.array([], dtype=int)
            self.bad_run_indexs = []
            ...
            self.parameter_searches = kwargs.pop('parameter_searches',10)
            self.log.debug("parameter_searches: "+str(self.parameter_searches))
            self.hyperparameter_searches = kwargs.pop('hyperparameter_searches',10)
            self.log.debug("hyperparameter_searches: "+str(self.hyperparameter_searches))
            self.bias_func_cycle = kwargs.pop('bias_func_cycle',4)
            self.bias_func_cost_factor = kwargs.pop('bias_func_cost_factor',[1.0,1.0,1.0,1.0])
            self.bias_func_uncer_factor = kwargs.pop('bias_func_uncer_factor',[0.0,1.0,2.0,3.0])
            self.search_precision = kwargs.pop('search_precision', 1.0e-6)
            self.bad_uncer_frac = kwargs.pop('bad_uncer_frac', 0.1)
        ...
        if self.generation_num < 3:
            self.log.info(str(self.remaining_kwargs))
            self.log.error('Number in generation must be larger than 2.: generation_num = '+str(self.generation_num))
            raise ValueError
    ...
    def get_params_and_costs(self):
        ...
        if self.all_params.size==0:
            ...
            self.all_valids = np.array(new_valids, dtype=int)
        else:
            ...
            self.all_valids = np.concatenate((self.all_valids, np.array(new_valids, dtype=int)))
        ...
        if self.all_valids.shape != (self.costs_count,):
            self.log('Saved GP uncertainties are the wrong size. THIS SHOULD NOT HAPPEN:' + repr(self.all_valids))
